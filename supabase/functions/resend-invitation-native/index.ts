...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[resend-invitation-native] 📧 Sending email via MailerSend to ${email}`);
    ...
    if (!emailResult.success) {
      console.error("[resend-invitation-native] ❌ MailerSend error:", emailResult.error);
      return new Response(
        JSON.stringify({ 
    ...
    console.log(`[resend-invitation-native] ✅ Email sent successfully via MailerSend`, { messageId: emailResult.messageId });

    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "Invitation renvoyée avec succès via MailerSend",
